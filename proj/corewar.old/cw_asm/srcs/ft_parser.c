...
/*   Updated: 2017/03/26 19:07:04 by sbenning         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ft_parse(self->left, scan, data))
	{
		if (ft_parse(self->right, scan, data))
			return (TRUE);
	}
	return (FALSE);
}
...
int                 match_or(t_parser *self, char **scan, void *data)
{
	if (ft_parse(self->right, scan, data))
		return (TRUE);
	return (ft_parse(self->left, scan, data));
}
...
int                 match_xor(t_parser *self, char **scan, void *data)
{
	...
	left = ft_parse(self->left, scan, data);
	right = ft_parse(self->right, scan, data);
